...
- A strict **Content-Security-Policy** is set on served assets: `default-src 'none'; script-src 'self'; style-src 'self'; connect-src 'self'; img-src 'self' data:`. This is why CSS and JS live in **separate embedded files with no inline** scripts or styles.
